...
class VeiculoForm(forms.Form):
    ...
    modelo = forms.ChoiceField(label='Modelo:', required=True, widget=forms.Select(attrs={'class': 'form-select form-select-sm'}))
    capacidade_tanque = forms.CharField(label='Capacidade Tanque:', initial=0, required=True, widget=forms.NumberInput(attrs={'class': 'form-control'}))
    combustivel = forms.ChoiceField(label='Combustivel:', choices=combustivel_choices, required=True, widget=forms.SelectMultiple(attrs={'class': 'form-select form-select-sm'}))
